...
export function availableColumnSizes(
  inputColumnSizes: Array<number>,
): Array<Object> {
  const validInputColumnSizes = validateInputColumnSizes(inputColumnSizes);
  const columnSizes = validInputColumnSizes || DEFAULT_COLUMN_SIZES;
  return columnSizes;
}

const validateInputColumnSizes = (inputColumnSizes) => {
  if (inputColumnSizes.length < 1) return;
  if (!inputColumnSizes.every((size) => Number(size) === size)) return;
  return inputColumnSizes.sort();
};
