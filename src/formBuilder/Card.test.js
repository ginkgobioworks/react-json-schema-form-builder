...
describe('Card', () => {
  ...
  it('renders with passed labels', () => {
    ...
    const placeHolderLabel = getHeadingText(wrapper, 3);
    expect(placeHolderLabel).toContain('Custom Input Type');
  });

  it('calls the onChange on placeholder', () => {
    const div = document.createElement('div');
    document.body.appendChild(div);
    ...
    const placeholder = wrapper
      ...
      .find('.card-text')
      .at(6);
    placeholder.simulate('change', { target: { value: 'wow title change' } });
