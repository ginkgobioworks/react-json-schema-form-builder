...
function getDefaultFormInputsWithMods(mods) {
  return { ...DEFAULT_FORM_INPUTS,
    ...(mods && mods.customFormInputs)
  };
}

export function defaultDataProps(category, mods) {
  const allFormInputs = getDefaultFormInputsWithMods(mods);
  return allFormInputs[category].defaultDataSchema;
}
export function defaultUiProps(category, mods) {
  const allFormInputs = getDefaultFormInputsWithMods(mods);
  return allFormInputs[category].defaultUiSchema;
}
export function categoryType(category, mods) {
  const allFormInputs = getDefaultFormInputsWithMods(mods);
  return allFormInputs[category].type;
}
export function getCardBody(category, mods) {
  const allFormInputs = getDefaultFormInputsWithMods(mods);
  return allFormInputs[category].cardBody;
}
export function categoryToNameMap(category, mods) {
  const allFormInputs = getDefaultFormInputsWithMods(mods);
...
export function generateCategoryHash(mods) {
  ...
  Object.keys(allFormInputs).forEach(categoryName => {
    ...
    formInput.matchIf.forEach(match => {
      match.types.forEach(type => {
        const hash = `type:${type || ""};widget:${match.widget || ""};field:${match.field || ""};format:${match.format || ""};$ref:${match.$ref ? "true" : "false"};enum:${match.enum ? "true" : "false"}`;
...
export function getCardCategory(cardProps, categoryHash) {
  ...
  if (!category) {
    if (cardProps.$ref) return "ref"; // eslint-disable-next-line no-console
...
const supportedPropertyParameters = new Set(["title", "description", "enum", "minLength", "maxLength", "multipleOf", "minimum", "maximum", "format", "exclusiveMinimum", "exclusiveMaximum", "type", "default", "pattern", "required", "properties", "items", "definitions", "$ref", "minItems", "maxItems", "enumNames", "dependencies", "$id"]);
const supportedUiParameters = new Set(["ui:order", "ui:widget", "ui:autofocus", "ui:autocomplete", "ui:option", "ui:field", "items", "definitions"]); // recursively called function to check an object for unsupported features
...
function checkObjectForUnsupportedFeatures(schema, uischema, supportedWidgets, supportedFields, supportedOptions) {
  ...
  const unsupportedFeatures = []; // check for unsupported whole object features
  ...
  if (uischema && typeof uischema === "object") Object.keys(uischema).forEach(uiProperty => {
    let propDefined = false; // search for the property in the schema properties and dependencies
  ...
  if (schema.properties) Object.entries(schema.properties).forEach(([parameter, element]) => {
    ...
    if (parameter !== correctName) {
      unsupportedFeatures.push(`Improper name '${parameter}': using '${correctName}' instead`);
    }

    if (element && typeof element === "object" && element.type && element.type !== "object") {
      // make sure that the type is valid
      if (!["array", "string", "integer", "number", "boolean"].includes(element.type)) unsupportedFeatures.push(`Unrecognized type: ${element.type} in ${parameter}`); // check the properties of each property if it is a basic object
    ...
    if (uischema && uischema[parameter] && element && (!element.type || element.type !== "object")) {
      ...
      Object.keys(uischema[parameter]).forEach(uiProp => {
        if (!supportedUiParameters.has(uiProp)) unsupportedFeatures.push(`UI Property: ${uiProp} for ${parameter}`); // check for unsupported ui widgets
        ...
        if (uiProp === "ui:field" && !supportedFields.has(uischema[parameter][uiProp])) unsupportedFeatures.push(`UI Field: ${uischema[parameter][uiProp]} for ${parameter}`); // check unsupported ui option

        if (uiProp === "ui:option") Object.keys(uischema[parameter]["ui:option"]).forEach(uiOption => {
          if (!supportedOptions.has(uiOption)) unsupportedFeatures.push(`UI Property: ui:option.${uiOption} for ${parameter}`);
        });
      });
    }
  });
...
export function checkForUnsupportedFeatures(schema, uischema, mods) {
  // add each unsupported feature to this array
  const unsupportedFeatures = []; // generate list of supported widgets, fields, options
  ...
  const supportedOptions = new Set(options); // check for unsupported whole form features
...
function generateElement(name, dataProps, uiProperties, requiredNames, definitionData, definitionUi, categoryHash) {
  let uiProps = { ...uiProperties
  };
  const newElement = {};
  let elementDetails = dataProps && typeof dataProps === "object" ? dataProps : {}; // populate newElement with reference if applicable
  ...
  if (elementDetails.type && elementDetails.type === "object") {
    ...
    newElement.uiOptions = uiProps || {}; // ensure that uiOptions does not have duplicate keys with dataOptions
...
export function generateElementPropsFromSchemas(parameters) {
  ...
  const elementDict = {};
  const requiredNames = schema.required ? schema.required : []; // read regular elements from properties

  Object.entries(schema.properties).forEach(([parameter, element]) => {
    const newElement = {};
    let elementDetails = element && typeof element === "object" ? element : {}; // populate newElement with reference if applicable
    ...
    if (elementDetails.$ref !== undefined && definitionData) {
      ...
      const definedUiProps = (definitionUi || {})[pathArr[2]];
      uischema[parameter] = { ...(definedUiProps || {}),
        ...uischema[parameter]
      };
    }
    ...
    if (elementDetails.type && elementDetails.type === "object") {
      ...
      // create a card
      newElement.dataOptions = elementDetails;
      newElement.uiOptions = uischema[parameter] || {}; // ensure that uiOptions does not have duplicate keys with dataOptions
        ...
        group.oneOf.forEach(possibility => {
          ...
          Object.entries(possibility.properties).forEach(([parameter, element]) => {
            ...
            if (!Object.keys(elementDict).includes(parameter)) {
              const newElement = generateElement(parameter, element, uischema[parameter], requiredNames, definitionData, definitionUi, categoryHash);
...
export function countElementsFromSchema(schemaData) {
  ...
  const elementDict = {};
  let elementCount = 0; // read regular elements from properties
  ...
  if (element.$ref !== undefined) {
    ...
    if (element.dataOptions.category === "section") {
      ...
      Object.keys(element.dataOptions).forEach(key => {
        if (!["category", "hideKey", "path", "definitionData", "definitionUi", "mods"].includes(key) && element.dataOptions[key] !== "") prop[key] = element.dataOptions[key];
      });
      return prop;
    }
...
export function getCardParameterInputComponentForType(category, mods) {
  const allFormInputs = getDefaultFormInputsWithMods(mods);
...
export function addCardObj(parameters) {
  ...
  const i = names.length > 0 ? Math.max(...names.map(name => name.startsWith("newInput") ? Number.parseInt(name.charAt(8), 10) : 0)) + 1 : 1;
  const newElement = {
    name: `newInput${i}`,
    required: false,
    dataOptions: {
...
export function generateElementComponentsFromSchemas(parameters) {
  ...
  const elementList = elementPropArr.map((elementProp, index) => {
    ...
    if (elementProp.propType === "card") {
      ...
      const TypeSpecificParameters = getCardParameterInputComponentForType(elementProp.dataOptions.category || "string", mods); // add a fully defined card component to the list of components
      ...
      return /*#__PURE__*/React.createElement(Card, {
        ...
        onChange: newCardObj => {
          ...
          Object.keys(newCardObj).forEach(propName => {
            ...
            } else if (!["name", "required", "lang", "neighborNames", "dependents", "dependent", "parent"].includes(propName)) {
          ...
          if (newElementObjArr[index].propType === "card") {
            ...
            newElementObjArr[index] = { ...oldElement,
      ...
      return /*#__PURE__*/React.createElement(Section, {
        ...
        onChange: (newSchema, newUiSchema, newRef) => {
          ...
          newElementObjArr[index] = { ...oldSection,
            schema: newSchema,
        ...
        onNameChange: newName => {
          const oldSection = elementProp; // check if newName overlaps with an existing name
...
function propagateElementChange(elementArr, definitionData, definitionUi, categoryHash) {
  ...
  elementArr.forEach(element => {
    ...
    if (element.propType === "section") {
      ...
      const newUiSchema = { ...element.uischema,
        ...generateUiSchemaFromElementProps(updatedChildren, definitionUi)
      };
      const newSchema = { ...element.schema,
