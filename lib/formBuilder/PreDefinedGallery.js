...
import { parse, stringify, propagateDefinitionChanges, generateCategoryHash } from "./utils";
...
export default function PreDefinedGallery({
  ...
  React.useEffect(() => {
    if (!uiSchemaData.definitions) {
      ...
      console.log("Parsing UI schema to assign UI for definitions"); // need to create definitions from scratch

      const references = []; // recursively search for all $refs in the schemaData
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    onChange: (newDefinitions, newDefinitionsUi) => {
      ...
      uiSchemaData.definitions = newDefinitionsUi; // propagate changes in ui definitions to all relavant parties in main schema

      propagateDefinitionChanges(schemaData, uiSchemaData, (newSchema, newUiSchema) => onChange(stringify(newSchema, lang), stringify(newUiSchema, lang)), categoryHash);
    },
    mods: mods,
