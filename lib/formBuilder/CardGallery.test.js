...
import CardGallery from "./CardGallery"; // mocks to record events
...
describe("CardGallery", () => {
  it("renders without error", () => {
    ...
    const wrapper = mount( /*#__PURE__*/React.createElement(CardGallery, props), {
      attachTo: div
    });
  ...
  it("renders appropriate number of cards with a give definition", () => {
    ...
    const div = document.createElement("div");
    document.body.appendChild(div);
    const wrapper = mount( /*#__PURE__*/React.createElement(CardGallery, modProps), {
      attachTo: div
    });
  ...
  it("calls the onChange method when editing a card", () => {
    ...
    const titleField1 = wrapper.find(".form-gallery-container").first().find(".card-text").at(2);
