import { parse, stringify, getCardCategory, checkForUnsupportedFeatures, generateElementPropsFromSchemas, generateSchemaFromElementProps, generateUiSchemaFromElementProps, generateCategoryHash } from "./utils";
const schema = {
  type: "object",
  properties: {
    obj1: {
...
describe("checkForUnsupportedFeatures", () => {
  ...
  it("gives warnings for unknown features in schema", () => {
    ...
    expect(checkForUnsupportedFeatures(testSchema, testUischema)).toEqual(["Unrecognized Object Property: erroneousKey"]);
  });
  it("gives warnings for unknown features in ui schema", () => {
    ...
    expect(checkForUnsupportedFeatures(testSchema, testUischema, "yaml")).toEqual(["Unrecognized UI schema property: ui:error", "UI Property: ui:error2 for obj1"]);
  });
});
describe("generateElementPropsFromSchemas", () => {
  it("generates an array of card objects from a schema and ui schema", () => {
    ...
    expect(cardObjArr).toHaveLength(3); // see if reading properties from data schema

    expect(cardObjArr[0].dataOptions.title).toEqual("testName2"); // see if reading type specific properties from data schema

    expect(cardObjArr[1].dataOptions.minLength).toEqual(5); // check if reading the ui schema

    expect(cardObjArr[2].uiOptions["ui:widget"]).toEqual("radio"); // check if order is correct
